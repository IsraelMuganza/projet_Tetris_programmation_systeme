...
public class PanneauTransparent extends JPanel{

    private Color bgColor = new Color(0, 0, 0, 200);

    public PanneauTransparent(){
        this.setLayout(null);
    ...
    public synchronized void gameOver(String message, int score, int lines, int levels, boolean keyClose, String[][] highest){
        ...
        if (inTopFive){
            if (message.equals("Game Over")){
                lb = new JLabel("Tu es dans le top 5!");
            } else {
                lb = new JLabel("Ton adversaire est dans le top 5!");
            }
            bgColor = new Color(120, 10, 30, 200);
        } else {
        ...
        if (keyClose){
            JLabel lb5 = new JLabel("Appuie un bouton pour fermer");
